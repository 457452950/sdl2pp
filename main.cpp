...
int main(int argc, char *argv[]) {
    ...
    for(auto &info : display_infos) {
        ...
        SDL_Log("display bounds : %d, %d, %d, %d",
                info.GetTotalBounds().x,
                info.GetTotalBounds().y,
                info.GetTotalBounds().w,
                info.GetTotalBounds().h);
        SDL_Log("display usable bounds : %d, %d, %d, %d",
                info.GetUsableBounds().x,
                info.GetUsableBounds().y,
                info.GetUsableBounds().w,
                info.GetUsableBounds().h);

        auto &supported_modes = info.GetSupportedModes();
        for(auto &mode : supported_modes) {
            SDL_Log("supported mode : %d, %d, %d, %s",
                    mode.w,
                    mode.h,
                    mode.refresh_rate,
                    SDL_GetPixelFormatName(mode.format));
