#pragma once
#ifndef SDL2PP_SDL2PP_BASE_SDLTTF_H_
#define SDL2PP_SDL2PP_BASE_SDLTTF_H_
...
namespace sdlpp {
    ...
    public:
        ...
        static std::shared_ptr<Font> Create(TTF_Font *&&font) {
            return std::shared_ptr<Font>(new Font(std::move(font)));
        }
        ...
        bool SetFontSize(int point_size) {
            return TTF_SetFontSize(font_, point_size) == 0;
        }
        ...
        Hinting GetHinting() {
            return static_cast<Hinting>(TTF_GetFontHinting(font_));
        }
        ...
        WrappedAlign GetWrapped() {
            return static_cast<WrappedAlign>(TTF_GetFontWrappedAlign(font_));
        }

        void SetWrapped(WrappedAlign wrapped) {
            TTF_SetFontWrappedAlign(font_, wrapped);
        }
        ...
        void SetKerning(int kerning) {
            TTF_SetFontKerning(font_, kerning);
        }
        ...
        std::string_view FaceFamilyName() {
            return {TTF_FontFaceFamilyName(font_)};
        }

        std::string_view FaceStyleName() {
            return {TTF_FontFaceStyleName(font_)};
        }

        bool IsGlyphProvided(uint16_t ch) {
            return TTF_GlyphIsProvided(font_, ch) != 0;
        }

        bool IsGlyphProvided(uint32_t ch) {
            return TTF_GlyphIsProvided32(font_, ch) != 0;
        }
        ...
        std::shared_ptr<SDLSurface>
        RenderGlyph_Solid(uint16_t ch, const SDL_Color &color) {
            return SDLSurface::Create(TTF_RenderGlyph_Solid(font_, ch, color));
        }

        std::shared_ptr<SDLSurface>
        RenderGlyph_Solid(Uint32 ch, const SDL_Color &color) {
            return SDLSurface::Create(TTF_RenderGlyph32_Solid(font_, ch, color));
        }

        std::shared_ptr<SDLSurface>
        RenderText_Shaded(std::string_view text, const SDL_Color &fg, const SDL_Color &bg) {
            return SDLSurface::Create(TTF_RenderText_Shaded(font_, text.data(), fg, bg));
        }

        std::shared_ptr<SDLSurface>
        RenderUTF8_Shaded(std::string_view text, const SDL_Color &fg, const SDL_Color &bg) {
            return SDLSurface::Create(TTF_RenderUTF8_Shaded(font_, text.data(), fg, bg));
        }

        std::shared_ptr<SDLSurface>
        RenderUNICODE_Shaded(std::string_view text, const SDL_Color &fg, const SDL_Color &bg) {
        ...
        std::shared_ptr<SDLSurface>
        RenderUNICODE_Shaded_Wrapped(std::string_view text, const SDL_Color &fg, const SDL_Color &bg,
                                     Uint32 wrapLength) {
            return SDLSurface::Create(
        ...
        std::shared_ptr<SDLSurface>
        RenderGlyph_Shaded(uint32_t glyph, const SDL_Color &fg, const SDL_Color &bg) {
            return SDLSurface::Create(TTF_RenderGlyph32_Shaded(font_, glyph, fg, bg));
        }

        std::shared_ptr<SDLSurface>
        RenderGlyph_Shaded(uint16_t glyph, const SDL_Color &fg, const SDL_Color &bg) {
            return SDLSurface::Create(TTF_RenderGlyph_Shaded(font_, glyph, fg, bg));
        }

        std::shared_ptr<SDLSurface>
        RenderText_Blended(std::string_view text, const SDL_Color &fg) {
            return SDLSurface::Create(TTF_RenderText_Blended(font_, text.data(), fg));
        }

        std::shared_ptr<SDLSurface>
        RenderUNICODE_Blended(std::string_view text, const SDL_Color &fg) {
            return SDLSurface::Create(TTF_RenderUNICODE_Blended(font_, (const uint16_t *) text.data(), fg));
        }

        std::shared_ptr<SDLSurface>
        RenderUTF8_Blended(std::string_view text, const SDL_Color &fg) {
        ...
        std::shared_ptr<SDLSurface>
        RenderGlyph_Blended(uint16_t ch, const SDL_Color &fg) {
            return SDLSurface::Create(TTF_RenderGlyph_Blended(font_, ch, fg));
        }

        std::shared_ptr<SDLSurface>
        RenderGlyph_Blended(uint32_t ch, const SDL_Color &fg) {
            return SDLSurface::Create(TTF_RenderGlyph32_Blended(font_, ch, fg));
        }


        std::shared_ptr<SDLSurface>
        RenderText_LCD(std::string_view text, const SDL_Color &fg, const SDL_Color &bg) {
            return SDLSurface::Create(TTF_RenderText_LCD(font_, text.data(), fg, bg));
        }

        std::shared_ptr<SDLSurface>
        RenderUTF8_LCD(std::string_view text, const SDL_Color &fg, const SDL_Color &bg) {
            return SDLSurface::Create(TTF_RenderUTF8_LCD(font_, text.data(), fg, bg));
        }

        std::shared_ptr<SDLSurface>
        RenderUNICODE_LCD(std::string_view text, const SDL_Color &fg, const SDL_Color &bg) {
        ...
        std::shared_ptr<SDLSurface>
        RenderUNICODE_LCD_Wrapped(std::string_view text, const SDL_Color &fg, const SDL_Color &bg,
                                  Uint32 wrapLength) {
            return SDLSurface::Create(
        ...
        std::shared_ptr<SDLSurface>
        RenderGlyph_LCD(uint32_t glyph, const SDL_Color &fg, const SDL_Color &bg) {
            return SDLSurface::Create(TTF_RenderGlyph32_LCD(font_, glyph, fg, bg));
        }

        std::shared_ptr<SDLSurface>
        RenderGlyph_LCD(uint16_t glyph, const SDL_Color &fg, const SDL_Color &bg) {
        ...
        bool SetDirection(Direction direction) {
            return TTF_SetFontDirection(font_, direction) == 0;
        }
...
#endif //SDL2PP_SDL2PP_BASE_SDLTTF_H_
