...
inline std::shared_ptr<SDL_Texture> MakeShared(SDL_Texture *&&texture) { return {texture, SDL_DestroyTexture}; }
...
public:
    ...
    // can be null
    bool GetSize(int *w, int *h) { return SDL_QueryTexture(texture_, nullptr, nullptr, w, h) == 0; }
    ...
    /**
     * Set color modulation, 调色，原色 {r, g, b}  --- {255/3, 255/4, 255/5} --->  {r/3, g/4, b/5}
     * @param rgb 忽略alpha值
     */
    bool SetColorMod(SDL_Color rgb) { return SDL_SetTextureColorMod(texture_, rgb.r, rgb.g, rgb.b) == 0; }

    bool GetColorMod(SDL_Color &rgb) { return SDL_GetTextureColorMod(texture_, &rgb.r, &rgb.g, &rgb.b) == 0; }
     ...
     * @return
     */
    bool SetAlphaMod(uint8_t alpha) { return SDL_SetTextureAlphaMod(texture_, alpha) == 0; }

    bool GetAlphaMod(uint8_t &alpha) { return SDL_GetTextureAlphaMod(texture_, &alpha) == 0; }

    bool SetBlendMode(BlendMode blend) { return SDL_SetTextureBlendMode(texture_, blend) == 0; }

    bool GetBlendMode(BlendMode &blend) { return SDL_GetTextureBlendMode(texture_, &blend) == 0; }

    bool SetScaleMode(ScaleMode scale) { return SDL_SetTextureScaleMode(texture_, scale) == 0; }

    bool GetScaleMode(ScaleMode &scale) { return SDL_GetTextureScaleMode(texture_, &scale) == 0; }

    bool SetUserData(void *userdata) { return SDL_SetTextureUserData(texture_, userdata) == 0; }
